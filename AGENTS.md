...
- **ASD-STE100 Simplified Technical English.** Write user-facing text in STE style: short sentences, one instruction per sentence, active voice, one meaning per word.
...
- **No silent workflow drift.** If a requested workflow, skill, MCP server, CLI, helper script, or documented step is unavailable, incompatible, failing, or cannot be used exactly as specified, stop before substituting a different workflow. Explain what the requested path required, what is actually available, what would change, and any downgrade in evidence quality or behavior. Continue only after the user explicitly approves the changed workflow; do not silently fall back to web search, memory, alternate tools, simplified procedures, or partial execution.
